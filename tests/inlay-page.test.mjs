...
const css = await readFile(new URL("../src/assets/inlay-page.css", import.meta.url), "utf8");
...
test("DEN-004 rendered polish stays page-specific", () => {
  assert.ok(html.includes('<link rel="stylesheet" href="/assets/inlay-page.css">'));
  assert.match(css, /td:last-child/);
  assert.match(css, /text-align:\s*left/);
  assert.match(css, /white-space:\s*normal/);
  assert.match(css, /> h3/);
  assert.ok(html.includes("<h3>Material on the quote</h3>"));
  assert.ok(html.includes("<h3>Surface count on the quote</h3>"));
});
