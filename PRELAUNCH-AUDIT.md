...
| K-002 | Query responsiveness | HIGH | DEN-002, DEN-005, DEN-008, DEN-010, DEN-018, DEN-025 | Canonical consolidation is preserved and the affected pages now answer the “without insurance” formulation explicitly without mislabeling broader publisher benchmarks as guaranteed cash prices. | Verify the new passages survive the Cloudflare build; do not create separate uninsured URLs. | IN PROGRESS |
| K-003 | Synonym coverage | MEDIUM | DEN-030 | Dentures now explains “dental plate” as common quote/search language for removable full or partial dentures while preserving dentures as the canonical entity. | Verify on preview. | IN PROGRESS |
...
| G-003 | Semantic internal-link graph | HIGH | Procedure pages / methodology | Contextual bridges were added from price, insurance and calculator passages to the matching methodology/support pages. Calculator Methodology now links back to all 15 supported procedure calculators. | Verify generated HTML and link resolution in the next Cloudflare build. | IN PROGRESS |
| G-004 | Architecture wording consistency | MEDIUM | Replacement / orthodontic / cosmetic related sections | Legacy “Related … cost guides” headings were replaced with neutral “Related … pages” wording to match the one-page-per-procedure architecture. | Verify on preview. | IN PROGRESS |
| T-003 | Reviewer/trust claim accuracy | HARD | Homepage | The site owner confirmed the current page set has been reviewed by the named Clinical & Scientific Reviewer. The hero signal is now the simpler factual wording “Clinically & scientifically reviewed”; hidden page-level reviewedBy schema remains blocked unless visible page-level credit is implemented. | Verify rendered wording and preserve documented review scope. | IN PROGRESS |
| T-004 | Brand/entity consistency | HIGH | Site + production metadata | “Dental Cost Calculator” is now the canonical site name across trust surfaces, WebSite schema, production title normalization and `og:site_name`. | Verify production artifact output. | IN PROGRESS |
...
| G-005 | Predicate strength | MEDIUM | Homepage hero | H1 is now “Understand what a dental procedure may cost,” aligning the predicate with published reference evidence rather than implying a normative fee. | Verify rendered hero and metadata alignment. | IN PROGRESS |
