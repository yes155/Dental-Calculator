...
| B-001 | Build/Deploy | HARD | Corrected implant code/content | Corrected-copy candidate `14f1bccec02e759e7d79eac5b3c324230fe97877` completed GitHub Actions `test-and-build` successfully, including `npm run qa`; frozen H2-order gate passed | Preserve automated gates; routine CI still applies to later control-only commits | GitHub Actions | PASS |
| B-002 | Cloudflare preview | HARD | Latest corrected implant pages | Cloudflare PR bot still reports earlier commit `fa921e62` as in progress; latest corrected-copy head not verified/rendered | Verify current head on Cloudflare and render pages | Cloudflare preview | IN PROGRESS |
...
| G6 Build/Security/Performance | IN PROGRESS | Corrected-copy GitHub build/security QA passes; performance and Cloudflare preview verification remain |
