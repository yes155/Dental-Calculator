...
|---|---|---|---|---|---|---|---|
| A-001 | Architecture | HARD | Site | Approved 38-URL architecture plus 5 deferred candidates is migrated to `data/page-registry.csv`; DEN-008 ownership frozen | Preserve registry ownership; do not create keyword-variant URLs | Registry review before new routes | PASS |
| E-001 | Evidence/Pricing | HARD | DEN-008 | Page separates CareCredit US averages from Humana Orlando examples; four DEN-008 sources are recorded in `data/source-register.csv` with scope/freshness notes | Keep source register synchronized with content changes | Source-to-claim reconciliation | PASS |
| E-002 | Evidence/Pricing | HARD | Remaining site | Site-wide source register/cost-data tables are not yet populated for remaining procedure pages | Build evidence packs/source rows before drafting each page | Cluster evidence QA | IN PROGRESS |
...
| C-001 | Calculator | HARD | CALC-008 | 13 shared + 9 extraction-specific fixtures pass; written spec now exists at `data/calculator-specs/CALC-008.md` | Keep fixtures/spec synchronized with implementation changes | `npm run qa` + spec diff | PASS |
...
| T-001 | Trust | HARD | Site | No false dental reviewer claim; author attribution present; trust routes are frozen in Page Registry | Build real trust/legal/methodology pages before release | Route/content/schema review | IN PROGRESS |
...
| B-001 | Build/Deploy | HARD | `chatgpt-work` | GitHub Actions Prelaunch QA and Cloudflare preview passed at earlier candidate `e3dea30b`; later data/control/spec commits require re-verification | Keep CI required for each candidate | GitHub Actions + preview deployment | IN PROGRESS |
...
| G1 Research/Architecture | PASS | Frozen Page Registry is now canonical in repository; deferred routes remain non-build |
| G2 Content/Trust | IN PROGRESS | DEN-008 representative page/evidence is controlled; trust surfaces and remaining pages incomplete |
...
| G5 Calculators/Data | IN PROGRESS | CALC-008 automated logic + written spec pass; rendered UX and remaining calculators/data are incomplete |
| G6 Build/Security/Performance | IN PROGRESS | Earlier build/CI/privacy baseline passes; current head and performance still require verification |
| G7 Preview | IN PROGRESS | Cloudflare preview path exists; current-head deployment and rendered preview verification still required |
...
**NO-GO for production.** Architecture control and the DEN-008 evidence/calculator audit trail are now in the repository. Remaining blockers include current-head CI/preview verification, visual/accessibility QA, site-wide evidence, supporting routes, trust surfaces, final technical SEO, media/performance and rollback.
