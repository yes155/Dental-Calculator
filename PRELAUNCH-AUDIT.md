...
| C-008 | Calculator UX | HIGH | CALC-003 / CALC-003-A04 | Approved CALC-001 interaction principles adapted to arch calculators without formula changes; 3-step flow, plain language, hidden IDs, progressive package disclosure | Verify latest Cloudflare render before final UX PASS | GitHub QA + rendered review | IN PROGRESS |
...
| D-002 | Design | HIGH | Arch calculators | CALC-003 and CALC-003-A04 now use constrained guided cards, plain-language steps and progressive package disclosure | Review latest desktop/mobile render | Multi-viewport review | IN PROGRESS |
...
| X-001 | Accessibility | HARD | Guided calculators | Native controls, visible labels, step focus targets, error summary, live result region and reduced-motion handling implemented; money fields visually group `$` with input | Run keyboard-only, focus-order, screen-reader and mobile checks | Browser/accessibility evidence | IN PROGRESS |
...
| B-001 | Build/Deploy | HARD | Current guided implant candidate | Guided arch implementation head `d1d5309d14c50b676165634ce6d26a94685dccb6` passed `test-and-build`; current control head `2f3fdad53c8970ff280f3b5e767b03d8326288a6` also passed | Preserve automated gates | GitHub Actions | PASS |
| B-002 | Cloudflare preview | HARD | CALC-003 / CALC-003-A04 | Cloudflare PR comment remains stale on an older commit; current guided arch render not yet confirmed | Verify latest branch deployment and rendered calculators | Cloudflare preview | IN PROGRESS |
...
| G-004 | Brief/source parity | HIGH | DEN-003 / DEN-012 | Brief heading wording reconciled to integrated page vectors and guided UX behavior | Preserve parity | Brief/build comparison | PASS |
...
| G3 Design/Media/Accessibility | IN PROGRESS | CALC-001 representative UX approved; arch guided UX implemented; multi-viewport/accessibility and media remain |
...
| G6 Build/Security/Performance | IN PROGRESS | Current GitHub QA/security controls pass; performance remains |
| G7 Preview | IN PROGRESS | CALC-001 representative UX approved by user; latest CALC-003/CALC-003-A04 deployment/render verification pending |
| G8 Rollback | NOT TESTED | Procedure pending |
| G9 Production | FAIL | Intentional block until hard gates pass |

## Current release decision

**NO-GO for production.** CALC-001 is now the approved representative calculator UX after iterative rendered review. CALC-003 and CALC-003-A04 have been adapted to the same plain-language three-step interaction principles without changing their frozen arch-normalization formulas or evidence boundaries, and GitHub QA passes. The next calculator gate is Cloudflare rendered review of those two arch calculators, followed by mobile/keyboard/accessibility checks. Trust pages, remaining evidence clusters, media, final technical SEO, performance and rollback remain open.
